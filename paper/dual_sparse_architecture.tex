...
The root cause is \textbf{functional overlap}---when two modules can independently explain the same variance in the training objective. The deeper mechanism is \textbf{learning-speed asymmetry}: when multiple modules can reduce the same loss term, the module with the shortest effective path from input to loss dominates early optimization. This collapses the error signal, starving slower or higher-capacity modules before they can specialize.

In our case, Engram's path (hash $\rightarrow$ embedding $\rightarrow$ output) is simpler than PEER's (query projection $\rightarrow$ product-key search $\rightarrow$ expert retrieval $\rightarrow$ output). Engram learns faster and captures patterns that PEER would otherwise specialize in. Output magnitude analysis confirms this: Engram dominates at a 2.62:1 ratio, effectively crowding out PEER's contribution.
...
Critically, these phases are \textit{soft biases in learning speed}, not strict stages---all modules remain plastic throughout training. The ``following'' module still adapts at $10^{-5}$ LR, maintaining compatibility with the leader's evolving representations.
...
\subsection{The Redundancy Bottleneck as Learning-Speed Asymmetry}

The Redundancy Bottleneck is a special case of \textbf{learning-speed asymmetry}: when multiple modules can reduce the same loss term, the module with the shortest effective path from input to loss dominates early optimization. As loss decreases, gradients to slower modules diminish, starving them of learning signal before they can specialize.
...
However, our experiments reveal \textbf{functional overlap} between PEER and Engram---both modules can independently explain the same variance in the training objective. Both attempt to ``retrieve'' information based on the input: Engram via literal n-gram hashing, PEER via learned query-key matching. Due to learning-speed asymmetry (Engram's simpler computation path), Engram dominates early optimization and captures patterns that PEER would otherwise specialize in.
